...
plt.subplot(131),plt.imshow(data[Nangles//2,:,:]), plt.xlabel('Angles'), plt.ylabel('x')
plt.subplot(132),plt.imshow(data[:,Ny//2,:]), plt.xlabel('Angles'), plt.ylabel('y')
...
theta = np.load(folder + 'sphere_phantom_360_theta_nodrift.npy')
...
plt.figure(figsize=(10,5))
plt.subplot(131),plt.imshow(sinogram_shifted[Nangles//2,:,:]), plt.xlabel('Angles'), plt.ylabel('x'), plt.colorbar()
plt.subplot(132),plt.imshow(sinogram_shifted[:,Ny//2,:]), plt.xlabel('Angles'), plt.ylabel('y'), plt.colorbar()
plt.subplot(133),plt.imshow(sinogram_shifted[:,:,Nx//2]), plt.xlabel('x'), plt.ylabel('y'), plt.colorbar()
plt.tight_layout()
...
plt.figure()
plt.subplot(131),plt.imshow(rec[:,:,rec.shape[2]//2]), plt.xlabel('x'), plt.ylabel('y')
plt.subplot(132),plt.imshow(rec[:,rec.shape[1]//2,:]), plt.xlabel('x'), plt.ylabel('z')
plt.subplot(133),plt.imshow(rec[rec.shape[0]//2,:,:]), plt.xlabel('y'), plt.ylabel('z')
plt.tight_layout()

# %% check the reprojection code
sinogram_model = tch.get_projections(rec, vol_geom, proj_geom)
plt.figure(figsize=(10,5))
plt.subplot(131),plt.imshow(sinogram_model[Nx//2,:,:]), plt.xlabel('Angles'), plt.ylabel('x'), plt.colorbar()
plt.subplot(132),plt.imshow(sinogram_model[:,Nx//2,:]), plt.xlabel('Angles'), plt.ylabel('y'), plt.colorbar()
plt.subplot(133),plt.imshow(sinogram_model[:,:,Nx//2]), plt.xlabel('x'), plt.ylabel('y'), plt.colorbar()
plt.tight_layout()
...
rec2 = tch.FBP_astra(data, vol_geom, proj_geom, weights)
data = data.transpose((0,2,1)) # transpose back

plt.figure()
plt.subplot(131),plt.imshow(rec2[:,:,rec2.shape[2]//2]), plt.xlabel('x'), plt.ylabel('y')
plt.subplot(132),plt.imshow(rec2[:,rec2.shape[1]//2,:]), plt.xlabel('x'), plt.ylabel('z')
plt.subplot(133),plt.imshow(rec2[rec2.shape[0]//2,:,:]), plt.xlabel('y'), plt.ylabel('z')
plt.tight_layout()

# %%
plt.figure(figsize=(20,5))
plt.subplot(131),plt.imshow(rec[180:250,Nx//2,100:180]), plt.xlabel('x'), plt.ylabel('z'), plt.title('Physically shifted'), plt.colorbar()
plt.subplot(132),plt.imshow(rec2[180:250,Nx//2,100:180]), plt.xlabel('x'), plt.ylabel('z'), plt.title('Geometry shifted'), plt.colorbar()
plt.subplot(133),plt.imshow(rec[180:250,Nx//2,100:180]-rec2[180:250,Nx//2,100:180]), plt.xlabel('x'), plt.ylabel('z'), plt.title('Difference'), plt.colorbar()
plt.tight_layout()
