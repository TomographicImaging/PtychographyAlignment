...
file = folder + '/275019_275199_tomo.nxs'
...
img_complex, theta, _ = utils_tomo.load_data(file, data_key= data_key, angle_key = angle_key)
img_complex = img_complex.transpose((1,2,0))

img = img_complex[int(object_ROI[0][0]):int(object_ROI[0][-1]+1),int(object_ROI[1][0]):int(object_ROI[1][-1]+1),:]
# remove one duplicate angle
img = img[:,:,1:]
theta = theta[1:]
[Ny, Nx, Nangles] = img.shape
tch.plot_3axes(img.real, 'img')
# %% Get shifts from matlab
...
# apply corrections from matlab
# for m in range(shifttt.shape[0]):
#     img[:,:,m] = np.roll(img[:,:,m],(int(shifttt[m,0]), int(shifttt[m,1])),axis=(1,0)) 

# img = tc.imshift_generic(img, shift=shifttt, Npix = None, affine_matrix = None, smooth = 0, 
#                                         ROI = None, downsample = 1, interp_method = 'circ', interp_sign = 1)
# tch.plot_3axes(img.real, 'img_roll')
# %%
...
for gag in range(1, Nangles):
    ...
    shift_ab = phase_cross_correlation(a,b,upsample_factor=100)
...
quick_correlation[:,0] = quick_correlation[:,0] - np.median(quick_correlation[:,0])
quick_correlation[:,1] = quick_correlation[:,1] - np.median(quick_correlation[:,1])
...
plt.legend()
# %%
# shift_total += quick_correlation
shift_total[:,0] = shift_total[:,0] + quick_correlation[:,0] # only apply horizontal

# for m in range(shifttt.shape[0]):
#     img[:,:,m] = np.roll(img[:,:,m],(int(shifttt[m,0]), int(shifttt[m,1])),axis=(1,0)) 

img = tc.imshift_generic(img, shift=shift_total, Npix = None, affine_matrix = None, smooth = 0, 
                                        ROI = None, downsample = 1, interp_method = 'circ', interp_sign = 1)
tch.plot_3axes(img.real, 'img_roll')
...
# %%
plt.figure(figsize=[10,3])
plt.subplot(121),plt.plot(theta, shift_total[:,1], label='x-correlation')
plt.subplot(121),plt.plot(theta, shifttt[:,1], '--', label='MATLAB code') 
plt.ylabel('Vertical shift (pixels)'), plt.xlabel('Angle (rad)')
plt.grid()
plt.subplot(122),plt.plot(theta, shift_total[:,0], label='x-correlation')
plt.subplot(122),plt.plot(theta, shifttt[:,0], '--', label='MATLAB code')
plt.ylabel('Horizontal shift (pixels)'), plt.xlabel('Angle (rad)')
plt.grid()
plt.legend()
# %% This is the 2D unwrapping
phase, residuals = tc.unwrap2D_fft2(img, empty_region=(50,50))
phase=phase.real

plt.imshow(phase[:,:,0]), plt.colorbar()
# %%
import importlib
import alignment.VerticalAlignment as VerticalAlignment

importlib.reload(VerticalAlignment)
# %%

config = VerticalAlignment.VerticalAlignmentConfig() # over-ride defaults here if needed
va = VerticalAlignment.VerticalAlignment(config)
shift_Y = va.run_alignment(phase, residuals, theta)
...
# %%
img_shift = tc.imshift_fft_ax(img, shift_Y, ax=0)
tch.plot_3axes(img_shift.real)
# %%
shift_total[:,1] = shift_total[:,1] + shift_Y
plt.figure(figsize=[10,3])
plt.subplot(121),plt.plot(theta, shift_total[:,1], label='x-correlation + VMF')
plt.subplot(121),plt.plot(theta, shifttt[:,1], '--', label='MATLAB code') 
plt.ylabel('Vertical shift (pixels)'), plt.xlabel('Angle (rad)')
plt.grid()
plt.subplot(122),plt.plot(theta, shift_total[:,0], label='x-correlation')
...
plt.legend()
# %%
img_orig = tc.imshift_generic(img_orig, shift=shift_total, Npix = None, affine_matrix = None, smooth = 0, 
                                        ROI = None, downsample = 1, interp_method = 'circ', interp_sign = 1)
# %%
...
ker2 = ker[np.newaxis,:,:]
convolution_result = ndimage.convolve((np.abs(img_orig_grad) > 2).astype(np.float32), ker2.astype(np.float32), mode = 'constant', cval = 0.0)
...
ROI = (
    slice(int(vert_range[0]) - 1, int(vert_range[-1]) + 1),
    slice(None)
)
...
for binning in bin_levels:
    ...
    sinogram = tc.imshift_generic(img_orig_grad, shift=np.zeros((img_orig.shape[-1],2)), Npix = None, affine_matrix = None, smooth = 5, 
                                        ROI = ROI, downsample = binning, interp_method = 'fft', interp_sign = 1)

    weights_find_shift = np.maximum(0,1-convolution_result)
    weights_find_shift = tc.imshift_generic(weights_find_shift, shift=np.zeros((img_orig_grad.shape[-1],2)), Npix = None, affine_matrix = None, smooth = 0, 
