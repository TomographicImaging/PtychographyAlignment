...
# vol_geom_perfect, proj_geom_perfect = tch.init_astra(Nx, Ny, np.deg2rad(theta))
# vol_geom, proj_geom = tch.init_astra(Nx, Ny, np.deg2rad(theta))
sinogram = data.copy()
...
for ii in range(iteration_no):
    ...
    MASS = np.median(sinogram * np.mean(abs(sinogram), axis=(0,1))) + shift_upd
...
plt.legend(['Calculated shifts', 'Simulated shifts'])
plt.grid()
