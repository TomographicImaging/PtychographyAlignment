...
for ii in range(iteration_no):
    ...
    cor = tc.find_cor(sinogram_shifted)
    ...
    if plot_figures: 
        plt.figure()
        plt.subplot(131),plt.imshow(rec_mask[:,:,rec.shape[2]//2])
        plt.subplot(132),plt.imshow(rec_mask[:,rec.shape[1]//2,:])
        plt.subplot(133),plt.imshow(rec_mask[rec.shape[0]//2,:,:])
